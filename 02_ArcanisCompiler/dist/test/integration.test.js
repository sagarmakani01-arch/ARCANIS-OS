...
function compileFull(source, sourceId = 'test.arc') {
...
`;
    ...
    it('supports emitting only specific stages', () => {
        ...
        const result = compiler.compile({ emitOnly: [types_1.CompilerStage.Lexing] });
        expect(result.stagesCompleted).toEqual([types_1.CompilerStage.Lexing]);
    });
