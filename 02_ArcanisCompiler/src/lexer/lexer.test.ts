...
function tokenize(source: string, sourceId = 'test.arc'): Token[] {
  const errors = new ErrorReporter();
  const lexer = new Lexer(source, sourceId, errors);
  return lexer.tokenize();
}

function tokenKinds(source: string): string[] {
...
describe('Lexer', () => {
  ...
  it('reports error on unterminated string', () => {
    const errors = new ErrorReporter();
    const lexer = new Lexer('"hello', 'test.arc', errors);
    lexer.tokenize();
  ...
  it('reports error on unexpected character', () => {
    const errors = new ErrorReporter();
    const lexer = new Lexer('@', 'test.arc', errors);
    lexer.tokenize();
    expect(errors.hasErrors()).toBe(true);
  });
